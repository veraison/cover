...
rim_matched if {
  ref := refvals[_]["element-list"][_]["value"]["integrity-registers"]["rim"]
  ev := evidence[_]["element-list"][_]["value"]["integrity-registers"]["rim"]
  ref == ev
}
...
ref_rems contains rem if {
  rem := {
    "name": name,
    "value": refvals[_]["element-list"][_]["value"]["integrity-registers"][name]["value"]
  }
  name != "rim"
}
...
ev_rems contains rem if {
  rem := {
    "name": name,
    "value": evidence[_]["element-list"][_]["value"]["integrity-registers"][name]["value"]
  }
  name != "rim"
}
...
executables := APPROVED_RT if {
  ...
  pv_matched
  rems_matched
} else := UNRECOGNIZED_RT if {
  rim_matched
  pv_matched
} else := UNRECOGNIZED_BOOT
